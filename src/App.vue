...
export default {
  ...
  methods: {
    generateDays(): void {
      ...
      // Generate 6 days (3 past days, today, 2 future days)
      for (let i = -3; i <= 2; i++) {
...
<template>
 <div class="relative" id="top">
  <header class="bg-foreground h-20 fixed z-50 w-full">
    <div>
      <h1 class="text-5xl text-center">ProjectCrane</h1>
    </div>
    <nav class="h-20">
      <ul class="flex gap-10 text-center justify-center pr-15 relative text-2xl">
        <li><a href="">To-Do app</a></li>
        <li><a href="">Money tracker</a></li>
        <li><a href="">Notes</a></li>
        <li><a href="">HealthCare tracker</a></li>
      </ul>
    </nav>
  </header>
        ...
        <aside class="w-full border-2 border-gray-500 h-dvh rounded-3xl">
          ...
          <div class="grid grid-cols-3 text-main p-5 gap-4">
            ...
            <div 
              v-for="day in days" 
              :key="day.date" 
              class="border-2 border-main rounded-lg p-4 bg-white shadow-sm"
            >
